...
(function () {
  ...
  const Icons = {
    ...
    claim: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><polyline points="17 11 19 13 23 9"/></svg>',
  ...
  let itemsGrid, searchInput, filterCategory, filterStatus, filterDate, clearFiltersBtn;
  ...
  function applyFilters() {
    ...
    if (currentFilters.category !== 'all') {
      filteredItems = filteredItems.filter(i => i.category === currentFilters.category);
    }

    // Status filter
    if (currentFilters.status !== 'all') {
      filteredItems = filteredItems.filter(i => i.status === currentFilters.status);
    }
    ...
    if (currentFilters.date !== 'all') {
      ...
      filteredItems = filteredItems.filter(i => {
        const d = i.date_found?.toDate ? i.date_found.toDate() : new Date(i.date_found);
  ...
  function bindEvents() {
    ...
    if (clearFiltersBtn) {
      clearFiltersBtn.addEventListener('click', () => {
        currentFilters = { category: 'all', status: 'all', date: 'all', search: '' };
